...
vis_save_root = '/work/weientai18/aissam_with_oracle_coco'
...
def vis(img_path, ais_box_all, sam_mask_all, oracle_mask_all, ais_mask_all, gt_mask_all, fp_idxs_all, small_idx, lm_idx):
    ...
    for size in ['small', 'lm']:
        ...
        for i in range(num_colors):
            ...
            omask = oracle_mask[i].squeeze(0).cpu().numpy().astype(np.uint8)
            gmask = gt_mask[i].squeeze(0).cpu().numpy().astype(np.uint8)
            amask = ais_mask[i].cpu().numpy().astype(np.uint8)
            smask = np.stack((b*smask, g*smask, r*smask), axis=2)
            omask = np.stack((b*omask, g*omask, r*omask), axis=2)
            amask = np.stack((b*amask, g*amask, r*amask), axis=2)
            gmask = np.stack((b*gmask, g*gmask, r*gmask), axis=2)
            sam_img = cv2.addWeighted(sam_img, 1, smask, 0.7, 0)
            oracle_img = cv2.addWeighted(oracle_img, 1, omask, 0.7, 0)
            ais_img = cv2.addWeighted(ais_img, 1, amask, 0.7, 0)
            gt_img = cv2.addWeighted(gt_img, 1, gmask, 0.7, 0)
        for i in range(num_colors):
            ais = ais_box[i]
            color = color_fp if i in fp_idxs else color_tp
            for image in [sam_img, ais_img, oracle_img, gt_img]:
                image = cv2.rectangle(image, (int(ais[0]), int(ais[1])), (int(ais[2]), int(ais[3])), color, 1)
        final_img = np.concatenate((gt_img, oracle_img), axis=0)
...
def main(args):
    ...
    for i, data in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            vis(img_path[0], ais_box_copy, pred_mask, oracle_mask, ais_mask, gt_mask, fp_idxs, small_idx, lm_idx)
